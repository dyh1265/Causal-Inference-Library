...
class Weighted_CFR(Model):
    def __init__(self, name, params, hp, **kwargs):
        ...
        self.hidden_phi = hp.Int('hidden_phi', min_value=16, max_value=512, step=16)
        self.fc = FullyConnected(n_fc=self.n_fc, hidden_phi=self.hidden_phi, final_activation='elu',
                                 out_size=self.params['hidden_phi'], kernel_init=params['kernel_init'], kernel_reg=None,
                                 name='fc')
        ...
        self.n_fc_t = hp.Int('n_fc_t', min_value=2, max_value=10, step=1)
        self.hidden_t = hp.Int('hidden_t', min_value=16, max_value=512, step=16)
        self.pred_t = FullyConnected(n_fc=self.n_fc_t, hidden_phi=self.hidden_t,
...
class CFRNet(CausalModel):
    ...
    def fit_tuner(self, x, y, t, seed):
        ...
        if self.params['ipm_type'] == 'weighted':
            monitor_metric = 'val_weighted_mmdsq_loss'
        elif self.params['ipm_type'] == 'mmdsq':
            monitor_metric = 'val_regression_loss'
        else:
            monitor_metric = 'val_cfrnet_loss'
        ...
        hypermodel = HyperCFRNet(params=self.params, pT=pT)
        ...
        if self.params['defaults']:
            ...
            if self.params['ipm_type'] == 'weighted':
                best_hps.values['n_fc_t'] = self.params['n_fc_t']
                best_hps.values['hidden_t'] = self.params['hidden_t']
    ...
    def fit_model(self, x, y, t, count, seed):
        ...
        if self.params['ipm_type'] == 'weighted':
            ...
            monitor_metric = 'regression_loss'
        else:
            monitor_metric = 'cfrnet_loss'
        ...
        if count == 0 and self.folder_ind == 0:
            parms_w = ""
            parms_s = f"""n_fc={best_hps.get('n_fc')} hidden_phi = {best_hps.get('hidden_phi')}
            ...
            print(f"""The hyperparameter are {parms_s}""")
            if self.params['ipm_type'] == 'weighted':
                params_w = f"""n_fc_t ={best_hps.get('n_fc_t')}, hidden_t={best_hps.get('hidden_t')} """
                print(f"""                     {params_w} """)

            self.sparams = parms_s + parms_w
            print(model.summary())
