...
def process_data_any_model(dataset_name, model_name_0, model_name_1, data_size, eye):
    layers = [1, 2, 3, 4]
    # read gnn4ite results eye

            # read gnn4ite results
    file_name_0 = 'results/result_' + model_name_0 + "_" + dataset_name + '.csv'
    results_0 = pd.read_csv(file_name_0)

    file_name_1 = 'results/result_' + model_name_1 + "_" + dataset_name + '.csv'
    results_1 = pd.read_csv(file_name_1)

    # for i, j in enumerate(train_sizes):
    data_0 = results_0[str(data_size)]
    data_1 = results_1[str(data_size)]
    m_test_0, h_test_0 = mean_confidence_interval(data_0, confidence=0.95)
    m_test_1, h_test_1 = mean_confidence_interval(data_1, confidence=0.95)

    # find t value
    print('Data size:', str(data_size), dataset_name)
    print('pehe_' + model_name_0 + ':', m_test_0, '+-', h_test_0)
    print('pehe_' + model_name_1 + ':', m_test_1, '+-', h_test_1)
    # Assuming you have two arrays of data: data1 and data2
    stat, p_value = ks_2samp(data_0, data_1)

    print("Kolmogorov-Smirnov test statistic:", stat)
    print("p-value:", p_value)
    stat, p_value = ttest_ind(data_0, data_1)
    print("t-test statistic:", stat)
    print("p-value:", p_value)
    result = anderson_ksamp([data_0, data_1])
    print("Anderson-Darling test statistic:", result.statistic)
    print("p-values:", result.significance_level, '\n')

    return m_test_0, m_test_1, h_test_0, h_test_1
